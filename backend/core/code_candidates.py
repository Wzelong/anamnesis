...
def match_fixed(resource_type: str, item: dict, effective=None) -> list[dict] | None:
    ...
    if resource_type == "Observation" and _mcode_obs_active(effective):
        tnm = match_tnm_category(
            item.get("value") or ""
        )  # value-specific; beats the broad "stage" term
        if tnm is not None:
            return tnm
        ...
        marker = match_tumor_marker_fixed(
            name
        )  # ER/PR/HER2/Ki-67 -> fixed LOINC (role tag still shapes value)
...
def _resolve_coding(
    effective, resource_type: str, item: dict
...
def _systems_for(
    resource_type: str, term_systems: list[str], pinned: dict[str, list[dict]]
) -> list[str]:
...
def _extract_search_terms(
    resource_type: str,
    item: dict,
    open_systems: list[str],
    code_body_site: bool = False,
    ...
    if resource_type == "AllergyIntolerance":
        ...
        if substance:
            terms.append(
                ("code", substance, item.get("substance_queries") or [substance], ["snomed"])
            )
...
async def _refine_query(
    prompt: str, term: str, system: str, client: genai.Client, model: str
) -> str | None:
    ...
    result = await parse_structured(
        ...
        CodeSelectorResult,
        stage="stage4",
        call_type=f"code_refine_{system}",
    )
...
async def _select_code(
    ...
    result = await parse_structured(
        ...
        CodeSelectorResult,
        stage="stage4",
        call_type=f"code_select_{system}",
    )
    if result is None:
        return None
    ...
    if result.refined_search_term:
        refined_results = await search_with_backoff(
            retriever, result.refined_search_term, system, 10
        )
        ...
        result2 = await parse_structured(
            ...
            CodeSelectorResult,
            stage="stage4",
            call_type=f"code_select_{system}_retry",
        )
...
def _collect_search_jobs(
    ...
    for ci, c in enumerate(candidates):
        ...
        for ti, (field, term, queries, systems) in enumerate(
            _extract_search_terms(c.resource_type, c.item, open_systems, cbs)
        ):
            syslist = (
                systems if field == "body_site" else _systems_for(c.resource_type, systems, pinned)
            )
...
def _subset_candidates(sys_codes: list[dict], queries: list[str], top_k: int) -> list[SearchResult]:
    ...
    return [
        SearchResult(
            code=c["code"], display=c.get("display") or "", score=1.0 - i * 0.02, rank=i + 1
        )
...
def _split_jobs(
    ...
    for job in jobs:
        ...
        open_systems, pinned = _resolve_coding(
            effective, candidates[ci].resource_type, candidates[ci].item
        )
...
async def _code_candidate(
    ...
    if candidate.resource_type == "AllergyIntolerance":
        ...
        if substance:
            ...
            sr = search_results.get(key, [])
            tasks.append(
                ("substance", _select_code(substance, "snomed", sr, client, model, retriever))
            )
            ti += 1
        if reaction:
            key = f"{ci}:{ti}:snomed"
            sr = search_results.get(key, [])
            tasks.append(
                ("reaction", _select_code(reaction, "snomed", sr, client, model, retriever))
            )
    ...
    for ti, (field, term, _queries, systems) in enumerate(search_terms):
        syslist = (
            systems
            if field == "body_site"
            else _systems_for(candidate.resource_type, systems, pinned)
        )
        for system in syslist:
            key = f"{ci}:{ti}:{system}"
            sr = search_results.get(key, [])
    ...
    if tasks:
        ...
        for (field, ti, _), r in zip(tasks, results, strict=False):
            if r is None:
                continue
            if field == "body_site":
    ...
    if not all_codings:
        term_name = item.get("full_name") or item.get("name") or item.get("substance") or ""
