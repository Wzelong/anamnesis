...
def test_tag_sets_role_when_active():
    out = _tag_mcode_roles(
        _obs_candidate("ER", "positive"), resolve_effective_profile(MCODE_PRESET)
    )
...
def _build(item):
    return build_fhir_resource(
        MergedCandidate(resource_type="Observation", item=item, source_refs=[]), "pt", {}
    )
    ...
    res = _build(
        {
            "name": "ER",
            "value": "positive",
            "coding": [{"system": LOINC, "code": "16112-5", "display": "Estrogen receptor"}],
            "mcode_role": "tumor-marker",
            "category": "laboratory",
        }
    )
    ...
    res = _build(
        {
            ...
            "value": "4.5",
            "unit": "ng/mL",
            "coding": [{"system": LOINC, "code": "2857-1", "display": "PSA"}],
            "mcode_role": "tumor-marker",
            "category": "laboratory",
        }
    )
    ...
    res = _build(
        {
            ...
            "value": "1.6",
            "coding": [{"system": LOINC, "code": "111004-8", "display": "AFP"}],
            "mcode_role": "tumor-marker",
        }
    )
...
def test_tumor_marker_profile_selected():
    res = {
        ...
        "code": {"coding": [{"system": LOINC, "code": "16112-5"}]},
    }
    ...
    res = _build(
        {
            ...
            "value": "IIA",
            "coding": [
                {"system": LOINC, "code": "21908-9", "display": "Stage group.clinical Cancer"}
            ],
            "category": "exam",
        }
    )
...
def test_tnm_category_clinical_vs_pathologic():
    assert match_tnm_category("T2") == [
        {"system": LOINC, "code": "21905-5", "display": "Primary tumor.clinical [Class] Cancer"}
    ]
...
def test_tnm_category_value_codeable_and_profiled():
    res = _build(
        {
            "name": "primary tumor category",
            "value": "pT3",
            "coding": [
                {
                    "system": LOINC,
                    "code": "21899-0",
                    "display": "Primary tumor.pathology [Class] Cancer",
                }
            ],
            "category": "exam",
        }
