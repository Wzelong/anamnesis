...
def _s2(types: list[str]) -> StageTwoOutput:
    return StageTwoOutput(
        document_id="d", note_context=NoteContext(), candidates={t: ["item"] for t in types}
    )


def test_no_op_when_all_enabled():
    out = _filter_disabled_types(
        [_s2(["Condition", "Procedure", "Observation"])], resolve_effective_profile(None)
    )
    ...
    eff = resolve_effective_profile(
        {
            ...
            "resources": {
                ...
                "AllergyIntolerance": {"enabled": False},
            },
        }
    )
