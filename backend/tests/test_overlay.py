...
def test_extension_without_value_not_stamped():
    out = apply_overlay(
        _condition(), ResourceRule(extensions=[{"url": "http://x/ext", "datatype": "string"}])
    )
    assert "extension" not in out
...
def test_extension_with_value_stamped_and_valid():
    rule = ResourceRule(
        extensions=[{"url": "http://x/eye-color", "datatype": "string", "value": "blue"}]
    )
...
def test_build_extension_datatypes():
    assert build_extension({"url": "u", "datatype": "boolean"}, True) == {
        "url": "u",
        "valueBoolean": True,
    }
    assert build_extension({"url": "u", "datatype": "integer"}, 3) == {
        "url": "u",
        "valueInteger": 3,
    }
    assert build_extension({"url": "u", "datatype": "CodeableConcept"}, "x") == {
        "url": "u",
