...
def test_builder_grade_value_codeable():
    res = build_fhir_resource(
        _obs(
            "histologic grade",
            "high grade",
            [{"system": NCIT, "code": "C18000", "display": "Grade"}],
        ),
        "pt",
        {},
    )
...
def test_builder_disease_status_codeable():
    res = build_fhir_resource(
        _obs(
            ...
            "stable",
            [{"system": LOINC, "code": "97509-4", "display": "Cancer disease status"}],
        ),
        "pt",
        {},
    )
...
def test_tumor_size_recognized_value_quantity_profiled():
    ...
    res = build_fhir_resource(
        MergedCandidate(
            ...
            item={
                ...
                "unit": "cm",
                "coding": [
                    {"system": LOINC, "code": "21889-1", "display": "Size.maximum dimension Tumor"}
                ],
                "category": "exam",
            },
        ),
    ...
    out = apply_specialty_profiles(
        {"resourceType": "Observation", "code": {"coding": [{"system": LOINC, "code": "21889-1"}]}},
        "Observation",
        cands,
    )
...
def test_spec_for_codings_roundtrip():
    assert (
        spec_for_codings([{"system": LOINC, "code": "89243-0"}])["profile"]
