...
async def run_arm(name, retriever, facts, variants, sem):
    ...
    async def one(fact):
        ...
        async with sem:
            ...
            var = await union_search(
                retriever, variants.get(fact["id"], [fact["text"]]), fact["system"]
            )
        sysk = fact["system"]
        by_sys_raw[sysk][1] += 1
        by_sys_var[sysk][1] += 1
    ...
    print(
        f"  + variants    {var_hits}/{n}  {var_hits / n * 100:5.1f}%   (lift {(var_hits - raw_hits) / n * 100:+.1f})"
    )
    for sysk in ("snomed", "icd10", "rxnorm", "loinc"):
        if by_sys_var[sysk][1]:
            ...
            print(
                f"    {sysk:<7} raw {rh:>2}/{rt:<2} {rh / rt * 100:5.1f}%  | +var {vh:>2}/{vt:<2} {vh / vt * 100:5.1f}%"
            )
    return var_hits / n
