...
def org(_id, name, line, city, state, zip_, phone, sysid):
    return {
        ...
        "id": _id,
        "meta": {"profile": prof("us-core-organization")},
        "identifier": [{"system": sysid, "value": _id.upper()}],
        "active": True,
        "name": name,
        "type": [
            cc(
        ...
        "address": [
            {"line": [line], "city": city, "state": state, "postalCode": zip_, "country": "US"}
        ],
    }
...
oakwood = org(
    ...
    "60601",
    "+1-312-555-0170",
    "http://oakwoodcancer.example.org/org-id",
)
lakeshore = org(
    "lakeshore",
    "Lakeshore Imaging",
    ...
    "60611",
    "+1-312-555-0190",
    "http://lakeshoreimaging.example.org/org-id",
)
...
def practitioner(_id, npi, family, given, prefix="Dr."):
    return {
        ...
        "id": _id,
        "meta": {"profile": prof("us-core-practitioner")},
...
patient = {
    ...
    "id": "sullivan-margaret",
    "meta": {"profile": prof("us-core-patient")},
    "extension": [
        {
            "url": f"{USCORE}/us-core-race",
            "extension": [
                {
                    "url": "ombCategory",
                    "valueCoding": {"system": CDCREC, "code": "2106-3", "display": "White"},
                },
                {"url": "text", "valueString": "White"},
            ],
        },
        {
            "url": f"{USCORE}/us-core-ethnicity",
            "extension": [
                {
                    "url": "ombCategory",
                    "valueCoding": {
    ...
    "identifier": [
        {
            "use": "usual",
            "type": cc(HL7_MR, "MR", "Medical Record Number"),
            "system": "http://oakwoodcancer.example.org/mrn",
            "value": "OAK-0117-SUL",
        }
    ],
    "active": True,
    "name": [{"use": "official", "family": "Sullivan", "given": ["Margaret"]}],
...
def problem(_id, codings, text, onset, recorded, recorder=PRAC_RAO, recorder_name="Dr. Priya Rao"):
    return {
        ...
        "code": cc_multi(codings, text),
        "subject": ref(PATIENT, PT_NAME),
        "onsetDateTime": onset,
        "recordedDate": recorded,
        "recorder": ref(recorder, recorder_name),
        "asserter": ref(recorder, recorder_name),
    }
...
cond_osteopenia = problem(
    ...
    [
        {"system": SCT, "code": "312894000", "display": "Osteopenia"},
        {
            "system": ICD10,
            "code": "M85.80",
            "display": "Other specified disorders of bone density and structure, unspecified site",
        },
    ],
    "Osteopenia",
    "2022-03-01",
...
def med(_id, code, display, dose_text, authored):
    return {
        ...
        "id": _id,
        "meta": {"profile": prof("us-core-medicationrequest")},
        "status": "active",
        "intent": "order",
        ...
        "subject": ref(PATIENT, PT_NAME),
        "authoredOn": authored,
        "requester": ref(PRAC_RAO, "Dr. Priya Rao"),
...
med_calvitd = med(
    "cal-vitd",
    "1303736",
    "Calcium Carbonate 600 MG / Cholecalciferol 400 unit Oral Tablet",
...
proc_biopsy = {
    ...
    "id": "core-biopsy",
    "meta": {"profile": prof("us-core-procedure")},
    "status": "completed",
    "code": cc(SCT, "122548005", "Biopsy of breast"),
    "subject": ref(PATIENT, PT_NAME),
    "performedDateTime": "2025-09-04",
...
# Encounters
def encounter(
    _id, class_code, class_disp, type_codings, type_text, start, end, prac, prac_name, sp
):
    return {
        "resourceType": "Encounter",
        "id": _id,
        "meta": {"profile": prof("us-core-encounter")},
        "identifier": [
            {
                "system": "http://oakwoodcancer.example.org/encounter-id",
                "value": f"ENC-{_id.upper()}",
            }
        ],
        "status": "finished",
        "class": {"system": HL7_ACT, "code": class_code, "display": class_disp},
        "type": [cc_multi(type_codings, type_text)],
        "subject": ref(PATIENT, PT_NAME),
        "participant": [
            {"individual": ref(prac, prac_name), "period": {"start": start, "end": end}}
        ],
        "period": {"start": start, "end": end},
        "serviceProvider": ref(sp),
    }


enc_consult = encounter(
    "onc-consult",
    "AMB",
    "ambulatory",
    [
    ...
    "2025-09-18T09:00:00-05:00",
    "2025-09-18T10:04:00-05:00",
    PRAC_RAO,
    "Dr. Priya Rao",
    ORG_OAKWOOD,
)
enc_surgery = encounter(
    "onc-surgery",
    "AMB",
    "ambulatory",
    [
    ...
    "2025-12-03T07:30:00-06:00",
    "2025-12-03T11:00:00-06:00",
    PRAC_VOSS,
    "Dr. Ellen Voss",
    ORG_OAKWOOD,
)
enc_followup = encounter(
    "onc-followup",
    "AMB",
    "ambulatory",
    [
    ...
    "2027-02-10T10:30:00-06:00",
    "2027-02-10T11:18:00-06:00",
    PRAC_RAO,
    "Dr. Priya Rao",
    ORG_OAKWOOD,
)
...
def doc(
    ...
    return {
        ...
        "id": _id,
        "meta": {"profile": prof("us-core-documentreference")},
        "identifier": [
            {
                "system": "http://oakwoodcancer.example.org/document-id",
                "value": f"DOC-{_id.upper()}",
            }
        ],
        "status": "current",
        "docStatus": "final",
        "type": cc(LOINC, loinc_code, loinc_disp),
        "category": [cc(US_DOC_CAT, "clinical-note", "Clinical Note")],
        "subject": ref(PATIENT, PT_NAME),
        "date": date_str,
        "author": [ref(author, author_name)],
        "custodian": ref(custodian, custodian_name),
        "content": [
            {
                "attachment": {
...
doc_consult = doc(
    ...
    "11488-4",
    "Consult note",
    "note-1-oncology-consultation.md",
    "2025-09-18T10:04:00-05:00",
    PRAC_RAO,
    "Dr. Priya Rao",
    ENC_CONSULT,
...
doc_path = doc(
    ...
    "11526-1",
    "Pathology study",
    "note-2-surgical-pathology.md",
    "2025-12-05T09:00:00-06:00",
    PRAC_VOSS,
    "Dr. Ellen Voss",
    ENC_SURGERY,
    ORG_OAKWOOD,
...
bundle = {
    "resourceType": "Bundle",
    "id": "anamnesis-demo-oncology-sullivan",
    "type": "transaction",
    "timestamp": "2027-03-01T08:30:00-06:00",
    "entry": [
        entry(ORG_OAKWOOD, oakwood),
        entry(ORG_LAKESHORE, lakeshore),
        entry(PRAC_RAO, prac_rao),
        entry(PRAC_VOSS, prac_voss),
        entry(PRAC_LIU, prac_liu),
        entry(PATIENT, patient),
        entry(COND_BREAST, cond_breast),
        entry(COND_HTN, cond_htn),
        entry(COND_OSTEOPENIA, cond_osteopenia),
        entry(MED_LISINOPRIL, med_lisinopril),
        entry(MED_CALVITD, med_calvitd),
        entry(OBS_MAMMO, obs_mammo),
        entry(PROC_BIOPSY, proc_biopsy),
        entry(ENC_CONSULT, enc_consult),
        entry(ENC_SURGERY, enc_surgery),
        entry(ENC_FOLLOWUP, enc_followup),
        entry(DOC_CONSULT, doc_consult),
        entry(DOC_PATH, doc_path),
        entry(DOC_FOLLOWUP, doc_followup),
    ],
}
