...
- **MCP server (`backend/po_main.py`)** — one FastMCP v3 server, one transport (Streamable HTTP at `/mcp`). One model-visible tool, `ReviewChart`, opens the in-host review workspace and seeds the patient header. The other twelve are app-only, invoked by the review app over the MCP-Apps postMessage bridge: the pipeline + write-back tools `RunExtraction` / `AcceptAugmentation` / `RejectAugmentation`; the terminology tools `SearchTerminology` (SNOMED / RxNorm / LOINC / ICD-10), `ResolveValueSet` (VSAC `$expand`), and `ParseCodes`; the prompt-tuning tools `DraftPromptAddon` / `TestPromptAddon` / `GetPromptBases`; and the config/usage tools `GetUserConfig` / `SetUserConfig` / `GetUsage`. SHARP-aware: FHIR base URL, access token, and patient id arrive as request headers.
...
- **BYOK, encrypted, required.** The pipeline runs on the clinician's own Gemini key (`gemini-3.7-flash`; guardrail on `gemini-3.1-flash-lite`), never a shared one. Secret fields are Fernet-encrypted at rest in the per-clinician config and only ever decrypted in-process; the iframe sees a `{set, last4}` presence flag.
